...
function SearchBar() {
    const navigate = useNavigate();  // navigated라고 하는 변수에 이동에 관련된 기능을 담아둬야 함
    ...
    const onSubmit = (event: SubmitEvent<HTMLFormElement>) => {
        ...
        // string의 맨 앞과 맨 뒤에 존재할 수있는 공백을 제거하는 메서드 :
        // "" 에서 사용자가 enter -> keyword.trim() -> ""
        // " " 에서 사용자가 enter -> keyword.trim() -> ""
        if (!keyword.trim()) return;    // trim을 했더니,값이 빈 스트림이면 return으로 끝내라

        // keyword를 활용해서 사용자를 강제 이동
        // search 라고 하는 경로에, query string으로 keyword를 전달해서 사용자 이름
        // URL은 한글이 X, 영어랑 몇 가지의 특수문자만 가능
        // 해석을 할 수있도록 영어랑 몇 가지의 특수문자를 이용해 변환 작업이 필요함 -> encodeURIComponent()
